...
- 能实现Redis集群[Redis集群配置、哨兵策略(案例)、Redis击穿问题]
...
1. 可扩展性：集群的性能不限制于单一的服务实体，新的服务实体可以动态的添加到集群，从而增强集群的性能。(动态拓展/静态拓展)
...
----返回redis根目录再次执行make MALLOC=libc即可----
```
...
1.解压redis压缩包
2.编译-> make
3.安装->指定安装路径->make install PREFIX=安装路径
```
...
```
1.准备6个节点   7001   7002   7003  7004 7005  7006
2.每个节点需要开启集群
3.需要用ruby将每个开启集群的节点串联到一起
```
...
```properties
./redis-trib.rb create --replicas 1 192.168.211.132:7001 192.168.211.132:7002 192.168.211.132:7003 192.168.211.132:7004 192.168.211.132:7005 192.168.211.132:7006
```
...
#### 4.7 redis缓存击穿问题解决(布隆过滤器--底层原理--查询资料)
...
​	基本的思路：通过redis缓存+mybatis的一级/二级缓存整合ehcache来实现。(mybatis一级缓存和二级缓存有什么区别?)
...
准备两台虚拟机，虚拟机的地址分别为192.168.211.134, 192.168.211.135
...
![1578491784550](images\1578491784550.png)

重启系统。

##### 5.3.1 安装erlang  

192.168.25211.134上安装如下：
...
![1578487090571](images\1578487090571.png)
...
安装方法如上边的方法进行在 `192.168.211.134`上进行
...
![1578498045293](images\1578498045293.png)

添加集群之后：

![1578498098221](images\1578498098221.png)
